...
def _build_live_record(keyword: str = "live") -> SampleRecord:
    return SampleRecord(sample_id="live_sample", keyword=keyword, path="", sample_rate=0, duration_ms=0, timestamp="live", session_id="live", num_samples=0, status="accepted")


def _mean_k_neighbor_distance(train_vectors_scaled: np.ndarray, query_vector_scaled: np.ndarray, *, k: int) -> float:
...
def _auto_unknown_distance_threshold(train_vectors_scaled: np.ndarray, *, k: int, percentile: float, margin: float) -> float:
    ...
    baseline = float(np.percentile(np.asarray(mean_neighbor_distances, dtype=np.float64), float(np.clip(percentile, 0.0, 100.0))))
...
def prepare_dynamic_classifier(project_root: Path, dynamic_source_root: Path):
    ...
    if dynamic_parts:
        ...
        dynamic_classifier = DynamicKeywordClassifier(k=DYNAMIC_K).fit(dynamic_parts, delta_map=delta_map if delta_map else None)
        dynamic_unknown_distance_threshold = (
            ...
            else _auto_unknown_distance_threshold(dynamic_classifier._train_vectors, k=dynamic_classifier.k, percentile=DYNAMIC_UNKNOWN_DISTANCE_PERCENTILE, margin=DYNAMIC_UNKNOWN_DISTANCE_MARGIN)
        )
...
def prepare_base_classifier(base_parts, *, model_type: str):
    ...
    if normalized_type == "knn":
        base_parts_aug = build_augmented_feature_parts(base_parts, project_root=PROJECT_ROOT, config=AugmentationConfig(copies_per_sample=3), seed=42)
        classifier = KeywordClassifier(k=BASE_K).fit(base_parts_aug)
        classifier = classifier.fit(base_parts)
        threshold = float(BASE_UNKNOWN_DISTANCE_THRESHOLD) if BASE_UNKNOWN_DISTANCE_THRESHOLD is not None else _auto_unknown_distance_threshold(classifier._train_vectors, k=classifier.k, percentile=BASE_UNKNOWN_DISTANCE_PERCENTILE, margin=BASE_UNKNOWN_DISTANCE_MARGIN)
        return classifier, float(threshold)
    if normalized_type == "svm":
        classifier = BaseSVMClassifier(kernel=BASE_SVM_KERNEL, C=BASE_SVM_C, gamma=BASE_SVM_GAMMA).fit(base_parts)
        return classifier, float(BASE_SVM_MARGIN_THRESHOLD)
    raise ValueError(f"Unsupported base model type: {model_type}")


def prepare_dual_live_model(*, project_root: Path, base_source_root: Path, dynamic_source_root: Path, base_model_type: str = BASE_MODEL_TYPE) -> DualLiveModel:
    ...
    base_classifier, base_unknown_distance_threshold = prepare_base_classifier(base_parts, model_type=base_model_type)
    dynamic_classifier, dynamic_unknown_distance_threshold = prepare_dynamic_classifier(project_root, dynamic_source_root)
    return DualLiveModel(base_source_root=Path(base_source_root).resolve(), dynamic_source_root=Path(dynamic_source_root).resolve(), base_classifier=base_classifier, base_model_type=str(base_model_type).strip().lower(), dynamic_classifier=dynamic_classifier, base_unknown_distance_threshold=float(base_unknown_distance_threshold), dynamic_unknown_distance_threshold=float(dynamic_unknown_distance_threshold), dynamic_uses_delta=bool(USE_DELTA_FOR_DYNAMIC_KNN))


def predict_base_command(model: DualLiveModel, samples: np.ndarray, sample_rate: int) -> BasePrediction:
    ...
    if model.base_model_type == "svm":
        ...
        return BasePrediction(bool(is_known), closest_label if is_known else None, closest_label, margin, float(model.base_unknown_distance_threshold), margin, float(model.base_unknown_distance_threshold), [], [])
    ...
    is_known = mean_neighbor_distance <= model.base_unknown_distance_threshold and label_vote_ratio >= BASE_MIN_LABEL_VOTE_RATIO
    return BasePrediction(bool(is_known), closest_label if is_known else None, closest_label, float(mean_neighbor_distance), float(model.base_unknown_distance_threshold), float(label_vote_ratio), float(BASE_MIN_LABEL_VOTE_RATIO), neighbor_labels, neighbor_distances)


def predict_dynamic_command(model: DualLiveModel, samples: np.ndarray, sample_rate: int) -> DynamicPrediction:
    if model.dynamic_classifier is None:
        return DynamicPrediction(False, None, "none", float("inf"), float(model.dynamic_unknown_distance_threshold), [], [])
    ...
    return DynamicPrediction(bool(is_known), action_label if is_known else None, closest_label, float(mean_neighbor_distance), float(model.dynamic_unknown_distance_threshold), list(prediction["neighbor_labels"]), list(prediction["neighbor_distances"]))
...
def _print_base_prediction(prediction: BasePrediction, *, model_type: str, k: int) -> None:
    ...
    if model_type == "svm":
        ...
        print(f"Closest base label was {prediction.closest_label} with decision margin {prediction.mean_neighbor_distance:.3f} (threshold {prediction.unknown_distance_threshold:.3f})")
        return
    neighbors = ", ".join(f"{label} ({distance:.3f})" for label, distance in zip(prediction.neighbor_labels, prediction.neighbor_distances))
    if prediction.is_known and prediction.predicted_label is not None:
        print(f"Base {model_name} prediction: {prediction.predicted_label}")
    else:
        print(f"Base {model_name} rejected the utterance.")
    print(f"Closest base label was {prediction.closest_label} at mean {k}-NN distance {prediction.mean_neighbor_distance:.3f} (threshold {prediction.unknown_distance_threshold:.3f})")
...
def _print_dynamic_prediction(prediction: DynamicPrediction, *, k: int) -> None:
    ...
    print(f"Closest dynamic action was {prediction.closest_label} at mean {k}-NN distance {prediction.mean_neighbor_distance:.3f} (threshold {prediction.unknown_distance_threshold:.3f})")
...
def run_live_dual_classification(*, model: DualLiveModel, config, project_root: Path) -> int:
    ...
    if default_device:
        print(f"Using input device {default_device.index}: {default_device.name} ({default_device.default_samplerate:.0f} Hz default)")
            ...
            while True:
                ...
                for event in endpointer.process_chunk(chunk):
                    if event.kind == "discarded":
                        if current_learning is not None:
                            print(f"Discarded short noise while learning {current_learning.batch.keyword} ({event.speech_ms} ms speech, {event.trailing_silence_ms} ms trailing silence).")
                        continue
                    if event.kind != "utterance" or event.audio is None or event.audio.size == 0:
                        continue
                    if current_learning is not None:
                        record, completed, is_complete = save_batch_utterance(event.audio, project_root=project_root, raw_dir=dynamic_root, manifest_path=manifest_path, batch=current_learning.batch, sample_rate=sample_rate)
                        print(f"Saved {record.sample_id} to {record.path} for {current_learning.batch.keyword} mapped to {current_learning.action_label} ({completed}/{current_learning.batch.total})")
                        if not is_complete:
...
def main() -> int:
    ...
    chosen_base_model = input(f"Choose base model [knn/svm] (default: {BASE_MODEL_TYPE}): ").strip().lower() or BASE_MODEL_TYPE
    model = prepare_dual_live_model(project_root=PROJECT_ROOT, base_source_root=Path(BASE_KEYWORD_SOURCE_ROOT).resolve(), dynamic_source_root=Path(DYNAMIC_KEYWORD_SOURCE_ROOT).resolve(), base_model_type=chosen_base_model)
    if DRY_RUN:
        dynamic_count = 0 if model.dynamic_classifier is None else len(model.dynamic_classifier._train_labels)
